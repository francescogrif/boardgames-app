...
function mapGame(g) {
  ...
  // genere/i
  const genre = Array.isArray(g.genre) ? g.genre : (g.genres || g.tags || g.tipo || []);
  const genreArr = Array.isArray(genre) ? genre
...
function renderCard(game){
  const p = game.players, d = game.duration;
  const meta = [
...
async function boot(){
  ...
  // ---- Wire events PRIMA del fetch (così la X funziona anche se il fetch fallisce)
  ...
  if (qClear) {
    qClear.addEventListener('pointerdown', onClear);
    qClear.addEventListener('click', onClear);
  }

  if (genre)    genre.addEventListener('change', (e)=>{ state.genre = e.target.value; update(); });
  if (players)  players.addEventListener('change',(e)=>{ state.players = e.target.value; update(); });
  if (duration) duration.addEventListener('change',(e)=>{ state.duration = e.target.value; update(); });
  if (complexity) complexity.addEventListener('change',(e)=>{ state.complexity = e.target.value; update(); });
  if (sort)     sort.addEventListener('change',   (e)=>{ state.sort = e.target.value; update(); });
