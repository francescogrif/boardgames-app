...
function mapGame(g) {
  ...
  // genere/i
  const genre =
    Array.isArray(g.genre)
      ? g.genre
      : g.genres || g.tags || g.tipo
      ? g.genres || g.tags || g.tipo
      : [];
  const genreArr = Array.isArray(genre)
    ? genre
    : typeof genre === 'string'
    ? genre.split(',').map((s) => s.trim())
    : [];
  ...
  return {
    ...
    bgg_url:
      g.bgg_url || (g.bgg_id ? `https://boardgamegeek.com/boardgame/${g.bgg_id}` : null),
...
function openModal(game) {
  ...
  $('#mMeta').innerHTML = [
    ...
    game.duration.min
      ? badge(
          `${game.duration.min}${game.duration.max ? `–${game.duration.max}` : ''} minuti`,
        )
      : '',
  ...
  const rules = $('#mRules');
  if (game.bgg_url) {
    bgg.href = game.bgg_url;
    bgg.style.display = 'inline-flex';
  } else {
    bgg.style.display = 'none';
  }
  if (game.rules_url) {
    rules.href = game.rules_url;
    rules.style.display = 'inline-flex';
  } else {
